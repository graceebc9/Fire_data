...
only works on sci8 server - needs lots of memory 
"""
...
output_path_final  = '/home/users/graceebc/Fire_data/MODIS/smaller_chunks/'
...
for year in year_list :
    ...
    for i in range(len(chunked_files)):
        file_name = '{0}_{1}_chunk.tif'.format(year, i)
        output_path =  output_path_final + file_name
        #check files not been run already 
        if file_name not in os.listdir(output_path_final):
            elements =[]
